...
class ByteOps {
    ...
    /** invalid */
    private static final int FMT_INVALID = 0;

    /** "-": {@code op} */
    private static final int FMT_NO_ARGS = 1;

    /** "0": {@code op}; implies {@code max_locals >= 1} */
    private static final int FMT_NO_ARGS_LOCALS_1 = 2;

    /** "1": {@code op}; implies {@code max_locals >= 2} */
    private static final int FMT_NO_ARGS_LOCALS_2 = 3;

    /** "2": {@code op}; implies {@code max_locals >= 3} */
    private static final int FMT_NO_ARGS_LOCALS_3 = 4;

    /** "3": {@code op}; implies {@code max_locals >= 4} */
    private static final int FMT_NO_ARGS_LOCALS_4 = 5;

    /** "4": {@code op}; implies {@code max_locals >= 5} */
    private static final int FMT_NO_ARGS_LOCALS_5 = 6;

    /** "b": {@code op target target} */
    private static final int FMT_BRANCH = 7;

    /** "c": {@code op target target target target} */
    private static final int FMT_WIDE_BRANCH = 8;

    /** "p": {@code op #cpi #cpi}; constant restricted as specified */
    private static final int FMT_CPI = 9;
     ...
     * local number
     */
    private static final int FMT_LOCAL_1 = 10;
     ...
     * local number
     */
    private static final int FMT_LOCAL_2 = 11;

    /**
     * "y": {@code op #byte} ({@code bipush} and
     * {@code newarray})
     */
    private static final int FMT_LITERAL_BYTE = 12;

    /** "I": {@code invokeinterface cpi cpi count 0} */
    private static final int FMT_INVOKEINTERFACE = 13;

    /** "L": {@code ldc #cpi}; constant restricted as specified */
    private static final int FMT_LDC = 14;

    /** "S": {@code sipush #byte #byte} */
    private static final int FMT_SIPUSH = 15;

    /** "T": {@code tableswitch ...} */
    private static final int FMT_TABLESWITCH = 16;

    /** "U": {@code lookupswitch ...} */
    private static final int FMT_LOOKUPSWITCH = 17;

    /** "M": {@code multianewarray cpi cpi dims} */
    private static final int FMT_MULTIANEWARRAY = 18;

    /** "W": {@code wide ...} */
    private static final int FMT_WIDE = 19;
    ...
    private static final int CPOK_Integer = 0x20;

    /** "F": flag bit for valid cp type for {@code Float} */
    private static final int CPOK_Float = 0x40;

    /** "J": flag bit for valid cp type for {@code Long} */
    private static final int CPOK_Long = 0x80;

    /** "D": flag bit for valid cp type for {@code Double} */
    private static final int CPOK_Double = 0x100;

    /** "c": flag bit for valid cp type for {@code Class} */
    private static final int CPOK_Class = 0x200;

    /** "s": flag bit for valid cp type for {@code String} */
    private static final int CPOK_String = 0x400;

    /** "f": flag bit for valid cp type for {@code Fieldref} */
    private static final int CPOK_Fieldref = 0x800;

    /** "m": flag bit for valid cp type for {@code Methodref} */
    private static final int CPOK_Methodref = 0x1000;

    /** "i": flag bit for valid cp type for {@code InterfaceMethodref} */
    private static final int CPOK_InterfaceMethodref = 0x2000;
